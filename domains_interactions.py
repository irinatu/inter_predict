...
if __name__ == '__main__':
    ...
    with open(opts.Domains) as csvfile:   
        ...
        if opts.Level == "":
            dom = {float(rows[3])/50000.0 :[rows[0], (float(rows[4])/50000.0)-1] for rows in reader if rows[2]== opts.Chrom}  
        else:
            #print "Podany level", opts.Level, opts.Chrom
            dom = {float(rows[3])/50000.0:[rows[0], (float(rows[4])/50000.0)-1] for rows in reader if (rows[1] == opts.Level and rows[2]==opts.Chrom)}
